import { aWss } from ".."
import { IContact, IdContact, MessageForCreatedUpdatedContact, MessageForDeletedContact, MethodForSend } from "../types/types"
...
export const sendAddedContact = async ({ _id, code, phone }: IContact) => {
  try {
  const message: MessageForCreatedUpdatedContact = { operation: MethodForSend.add, _id, code, phone }
    broadcastConnection(JSON.stringify(message));
  } catch (error) {
    console.log(error);
  }
...
export const sendUpdatedContact = async ({ _id, code, phone }: IContact) => {
  try {
    const message: MessageForCreatedUpdatedContact = { operation: MethodForSend.update, _id, code, phone }
    broadcastConnection(JSON.stringify(message));
  } catch (error) {
    console.log(error);
  }
